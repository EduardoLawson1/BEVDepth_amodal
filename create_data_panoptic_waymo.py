...
def panoptic_waymo_data_prep(root_path, info_prefix, version, max_sweeps=10, debug=True):
    ...
    if split == 'train':
        split_path = os.path.join(root_path, 'training')
    elif split == 'val':
        split_path = os.path.join(root_path, 'validation')
    
    # camera_names = {'image_2':'P2', 'image_3':'P3'}
    camera_names = {'front':1, 'front_left':2, 'front_right':3, 'side_left':4, 'side_right':5}
    
    data_infos = {'infos': [], 'metadata': {'version':version}}
    for seq_id in os.listdir(split_path):
        print(f'Processing seq {seq_id}')
        lidar_dir = os.path.join(split_path, seq_id, 'velodyne')
        lidar_files = sorted(os.listdir(lidar_dir))
        calib_path = os.path.join(split_path, seq_id, 'calib')
        for i, lidar_file in enumerate(lidar_files):
            info = {}
            lidar_path = os.path.join(lidar_dir, lidar_file)
            info['lidar_path'] = lidar_path
            info['scene_token'] = f'{seq_id}'
            frame_id = lidar_file.split('.')[0]
            info['token'] = frame_id + f'_{seq_id}'
            info['lidar_token'] = info['token']
            seq_id_ = f'{seq_id}'
            label_id =  frame_id
            pan_gt_path =  osp.join(split_path, seq_id, 'panoptic', f"{label_id}.npz")
            caliberation = mmcv.load(os.path.join(calib_path, f"{label_id}.pkl"), file_format='pkl')


            info['sweeps'] = []
            info['timestamp'] = caliberation[1]['timestamp']
            info['cams'] = {}
            for cam, cam_key in camera_names.items():
                # cam_path = lidar_file.replace('velodyne', cam).replace('.bin', '.png')
                cam_path = os.path.join(split_path, seq_id, cam,  f"{label_id}.jpg")
                info['cams'][f'CAM_{cam.upper()}'] = {
                    'data_path': cam_path,
                    'type': f'CAM_{cam.upper()}',
                    'sample_data_token': info['token'],
                    'sensor2lidar_rotation': caliberation[cam_key]['sensor2lidar_rotation'],
                    'sensor2lidar_translation': caliberation[cam_key]['sensor2lidar_translation'],
                    'sensor2ego_translation': caliberation[cam_key]['sensor2ego_translation'],
                    'sensor2ego_rotation': caliberation[cam_key]['sensor2ego_rotation'],
                    'ego2global_rotation': caliberation[cam_key]['ego2global_rotation'],
                    'ego2global_translation': caliberation[cam_key]['ego2global_translation'],
                    'cam_intrinsic': caliberation[cam_key]['cam_intrinsic'],
                    'timestamp': info['timestamp']
                }
            info['lidar2ego_rotation'] = caliberation[1]['lidar2ego_rotation']
            info['lidar2ego_translation'] = caliberation[1]['lidar2ego_translation']
            info['ego2global_rotation'] = caliberation[1]['ego2global_rotation']
            info['ego2global_translation'] = caliberation[1]['ego2global_translation']
            info['gt_boxes'] = []
            info['gt_names'] = []
            info['num_lidar_pts'] = []
            info['num_radar_pts'] = []
...
if __name__ == '__main__':
    dataset = 'nuscenes'
    version = 'v2.0'
    train_version = f'{version}-train'
    root_path = '/home/mohan/datasets/waymo/panoptic_waymo/'
    extra_tag = 'panoptic_waymo-v3.0'
    # l = mmcv.load(
    #     '/home/mohan/hpcgpu11/occformer_data/SemanticKITTI/semantic_kitti-v3.0_infos_val.pkl')
    panoptic_waymo_data_prep(
        root_path=root_path,
